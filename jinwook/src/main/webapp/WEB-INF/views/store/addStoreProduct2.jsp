...
<script type="text/javascript">
	...
	function fncDeleteStoreProduct(e) {
		...
		var STORENO2 = $("input[id='storeNo']").val();
		...
		$.ajax({
			...
			success : function(result) {
				if (result != null) {
					alert("삭제완료");
					self.location = "addStoreProduct?storeNo=" + STORENO2;
				}
			}
	...
	$(function() {
		$(".btn:contains('수정')").on(
				...
				function fncUpdateStoreProduct() {

					STORENO = $(".btn:contains('수정')").data('store_no');
					PRODNO = $(".btn:contains('수정')").data('prod_no');
					PRODINFO = $(".btn:contains('수정')").data('prod_info');
					PRICE = $(".btn:contains('수정')").data('price');
					PRODIMG = $(".btn:contains('수정')").data('prod_img');
					PRODORIGN = $(".btn:contains('수정')").data('prod_orign');

					alert(PRODNO + PRICE + PRODIMG + PRODORIGN);
												...
												<c:forEach var="store" items="${store}">
													<div class="row" style="width: 900px;">
														...
														<button type="button" id="update"
															data-prod_no="${store.product.prodNo}"
															data-price="${store.product.price}"
															data-prod_img="${store.product.prodImg}"
															data-prod_info="${store.product.prodInfo}"
															data-prod_orign="${store.product.prodOrign}"
															data-store_no="${store.storeNo}" name="buttons"
